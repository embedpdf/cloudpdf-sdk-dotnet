...
public readonly record struct DocPagesExtract404ResponseCode : IStringEnum
{
    ...
    public static readonly DocPagesExtract404ResponseCode SigningPending = new(
        Values.SigningPending
    );

    public static readonly DocPagesExtract404ResponseCode SigningExpired = new(
        Values.SigningExpired
    );

    public static readonly DocPagesExtract404ResponseCode SigningVersionMismatch = new(
        Values.SigningVersionMismatch
    );

    public static readonly DocPagesExtract404ResponseCode SignatureRefused = new(
        Values.SignatureRefused
    );

    public static readonly DocPagesExtract404ResponseCode ProtectedDocument = new(
        Values.ProtectedDocument
    );

    public static readonly DocPagesExtract404ResponseCode StaleBase = new(Values.StaleBase);
